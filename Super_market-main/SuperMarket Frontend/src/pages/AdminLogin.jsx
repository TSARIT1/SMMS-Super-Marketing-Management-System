...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!backendUp) {
      toast.error("Backend server is unreachable. Please try again later.");
      return;
    }
    ...
    try {
      ...
      if (isNetworkError) {
        toast.error(
          "Cannot reach backend server. Please try again later.",
        );
      } else {
      ...
      <div className="bg-white shadow-xl rounded-2xl w-full max-w-md p-8 border-t-4 border-blue-500 animate-fadeIn">
        ...
        {!backendUp && (
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-800">
            Backend server is unreachable. <strong>Please try again later</strong> or contact support.
            <button
